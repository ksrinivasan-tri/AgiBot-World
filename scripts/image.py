...
from datasets.utils.py_utils import first_non_null_value, no_op_if_value_is_null, string_to_dict
...
class Image:
    ...
    def decode_example(self, value: dict, token_per_repo_id=None) -> "PIL.Image.Image":
        ...
        if bytes_ is None:
            ...
            # When loading from bytes, create a BytesIO object
        ...
        # For images loaded from bytes, we need to handle EXIF differently
        # since the file pointer might be None after load()
        if bytes_ is not None:
            # For byte data, we need to reopen the image to access EXIF
            # since the original file pointer might be closed
            try:
                # Get EXIF orientation from a fresh BytesIO to ensure file pointer is valid
                bytes_io_for_exif = BytesIO(bytes_)
                temp_img = PIL.Image.open(bytes_io_for_exif)
                exif = temp_img.getexif()
                if exif and exif.get(PIL.Image.ExifTags.Base.Orientation) is not None:
                    image = PIL.ImageOps.exif_transpose(image)
                # Close the temporary image
                temp_img.close()
                bytes_io_for_exif.close()
            except Exception:
                # If there's any issue with EXIF handling, continue with the image as is
                pass
        else:
            # For file-based images, use the standard approach
            try:
                exif = image.getexif()
                if exif and exif.get(PIL.Image.ExifTags.Base.Orientation) is not None:
                    image = PIL.ImageOps.exif_transpose(image)
        ...
        # Make sure to close the BytesIO object if we created one
        if bytes_ is not None and 'bytes_io' in locals():
            bytes_io.close()
